...
function Contact({ contact, setSearchResults, socket }) {
  ...
  const openConversation = async () => {
    const values = {
      ...
      token,
      isGroup: false, // ✅ thêm vào để tránh lỗi 400
    };

    try {
      let newConvo = await dispatch(open_create_conversation(values)).unwrap();
      socket.emit("join conversation", newConvo._id);
    } catch (error) {
      console.error("Failed to open conversation:", error);
      // Optionally: Hiển thị thông báo lỗi cho người dùng
    }
  };
